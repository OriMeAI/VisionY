...
const ObtainedLogContent: React.FC<IProps> = () => {
  ...
  const columns: TableColumnsType<ObtainedRecordDataItemType> = [
    ...
    {
      title: t("obtained_credits"),
      dataIndex: "quantity",
      align: 'center',
      render: (text, record) => {
        return (
          <span>{`${record.quantity > 0 ? '+' : '-'}${Math.abs(record.quantity)}`}</span>
        );
      },
    },
  ];
  ...
  const [records, setRecords] = useState<ObtainedRecordDataItemType[]>([]);
  ...
  const loadMoreData = useCallback(async (pageNum: number, isInitial = false) => {
    ...
    try {
      const res = await userApi.getObtainedRecordByPage({
        pageSize: pageSize,
        pageNum: pageNum,
      });
      ...
      console.error("Load obtained log failed:", error);
    } finally {
      setLoading(false);
    ...
    <div className="flex-1 flex flex-col w-full space-y-6">
      ...
      <div className="flex-1 flex flex-col" ref={containerRef}>
        <Spin spinning={initialLoading} tip={t('loading_more')}>
          <Table<ObtainedRecordDataItemType>
            columns={columns}
            dataSource={records.map((record,index) => ({
              ...record,
              obtainedTime: new Date(record.obtainedTime).toLocaleString('sv-SE'),
              key: record.obtainedTime + index,
            }))}
            pagination={false}
...
export default ObtainedLogContent;