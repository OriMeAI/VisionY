...
const PlaybackControls: React.FC<PlaybackControlsProps> = ({
  ...
  return (
    <div
      ...
      className="flex flex-wrap md:flex-nowrap items-center space-x-0 md:space-x-3 px-5 py-4 bg-gray-100 bg-opacity-90 rounded-lg shadow-sm"
    >
      {/* 播放速度选择 - 左侧 */}
      <div
      ...
      >
        {/* 编辑台词按钮 */}
        <button
          id="dialogue-edit-btn"
          className={`flex items-center space-x-1 px-3 py-1.5 mx-3 rounded-md text-xs transition-all bg-primary text-white`}
          onClick={() => {
            openDialogueEditModal && openDialogueEditModal();
          }}
        >
          <span>{t("edit_dialogue")}</span>
        </button>
        <button
